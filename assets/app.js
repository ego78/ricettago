...
async function openRecipe(id){try{const r=await api("getRecipe",{id});$("#recipeDetail").innerHTML=`${r.imageUrl?`<img class="detailImg" src="${r.imageUrl}">`:""}<h1>${esc(r.title)}</h1><div class="chips"><span class="chip">⏱ ${r.time||"—"}</span><span class="chip">👥 ${r.servings||"—"} porzioni</span><span class="chip">🔥 ${r.calories||"—"} kcal</span><span class="chip">P ${r.protein||"—"}g</span><span class="chip">C ${r.carbs||"—"}g</span><span class="chip">G ${r.fat||"—"}g</span></div>${r.source?`<a class="sourceBtn" href="${esc(r.source)}" target="_blank" rel="noopener noreferrer">▶ Apri video / fonte originale</a>`:""}<h2>Ingredienti</h2><ul class="ingredients">${(r.ingredients||[]).map(x=>`<li>${esc(x)}</li>`).join("")}</ul><h2>Procedimento</h2><ol class="steps">${(r.steps||[]).map(x=>`<li>${esc(x)}</li>`).join("")}</ol><button class="secondary" onclick="deleteRecipe('${r.id}')">Elimina ricetta</button>`;showPage("recipe")}catch(e){alert(e.message)}}
